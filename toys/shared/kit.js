/* exported initTool, renderSettings, getSettings, createTable, inputLines, runTool */

/* ==========================================================================
   GAN Toys — kit compartilhado dos toys de texto.

   Estes toys são a mesma ferramenta com um CONFIG diferente: o kit traz a
   casca (textarea, botão Processar, campos de configuração, tabela de
   resultado, cópia e limpeza) e os transformadores; o script do toy declara
   só o tipo que ele usa (`initTool(CONFIG)`).

   Antes cada toy carregava uma cópia inteira deste arquivo (oito cópias de
   ~22 KB) e elas já tinham divergido: uma tinha a correção do escape de
   classe em `normalizeAscii` e a regra nova de `;` em `normalizeNameValue`,
   as outras não. Aqui vale a versão corrigida.

   Carregado como script clássico antes do script do toy: a ordem importa,
   porque o script do toy chama `initTool` no carregamento.
   ========================================================================== */
function initTool(config) {
    ...
    clearButton.addEventListener('click', () => {
        ...
        settings.querySelectorAll('input, select').forEach((field) => {
            // istanbul ignore next -- idem: o kit não gera campos checkbox
            if (field.type === 'checkbox') field.checked = false;
            else if (field.tagName === 'SELECT') field.selectedIndex = 0;
            else field.value = field.defaultValue;
        });
        settings.querySelector('[name="mode"]')?.dispatchEvent(new Event('change'));
    ...
    function renderResult(result) {
        ...
        const copyRows = skipFirstColumn
            ...
            : result.rows;
        copiedText = copyRows
            // istanbul ignore next -- nenhum transformador devolve célula nula
...
function renderSettings(config, container) {
    const options = {
        ...
        'strip-suffix': [
            { key: 'mode', label: 'Modo', type: 'select', value: 'remove', choices: [['remove', 'Remover'], ['replace', 'Substituir']] },
            { key: 'pattern', label: 'Padrão (aceita * e ?)', value: '-*' },
            { key: 'replacement', label: 'Substituir por', value: '', conditionalOn: 'replace' }
        ]
    }[config.type] || [];
    ...
    options.forEach((option) => {
        /* istanbul ignore next -- nenhum CONFIG do kit declara opção do tipo checkbox hoje */
        if (option.type === 'checkbox') {
            const checkRow = document.createElement('label');
            checkRow.className = 'check-row';
            // O rótulo vem do CONFIG do toy, mas o campo é montado com DOM e
            // textContent: nada de interpolar valor em HTML.
            const check = document.createElement('input');
            check.type = 'checkbox';
            check.name = option.key;
            const rotulo = document.createElement('span');
            rotulo.textContent = option.label;
            checkRow.append(check, rotulo);
    ...
    if (mode) {
        const updateConditionalFields = () => {
            container.querySelectorAll('[data-conditional-on]').forEach((field) => {
                field.classList.toggle('hidden', mode.value !== field.dataset.conditionalOn);
            });
        };
...
function normalizeIris(text) {
    function clean(value) {
        // istanbul ignore next -- inputLines já descarta linha vazia
...
function runTool(type, text, settings) {
    ...
    if (type === 'strip-suffix') return transformByWildcard(text, settings.pattern, settings.mode, settings.replacement);
...
function normalizeNames(text) {
    ...
    if (!lines.length) return { headers: ['Normalizado'], rows: [] };

    const rows = lines.map((line) => {
        const cols = line.split('\t');
        return cols.map(normalizeNameValue);
    });
    ...
    const headers = maxCols === 1
        ? ['Normalizado']
        : Array.from({ length: maxCols }, (_, index) => `Normalizado ${index + 1}`);
...
function resolveCarteiraReu(text) {
    ...
    // istanbul ignore next -- norm() só é chamado com as strings das tabelas
    ...
    const find = (query) => {
        ...
        refs.forEach((ref) => {
            ...
            variants.forEach((variant) => {
                let candidate = 0;
                // O valor do réu já é resolvido acima (exactReu), então aqui basta a carteira.
                if (variant === norm(ref[0])) candidate = 10000;
...
function transformByWildcard(text, pattern = '-*', mode = 'remove', replacement = '') {
    const normalizedPattern = String(pattern || '');
    if (!normalizedPattern) return { error: 'Informe um padrão.' };

    const escapedPattern = [...normalizedPattern].map((character) => {
        if (character === '*') return '.*';
        if (character === '?') return '.';
    ...
    let matcher;
    try {
        matcher = new RegExp(escapedPattern, 'g');
    } catch (error) {
        return { error: 'O padrão informado não pôde ser processado.' };
    }

    const replacementText = mode === 'replace' ? String(replacement ?? '') : '';
    return {
        headers: ['Original', mode === 'replace' ? 'Substituído' : 'Sem trecho'],
        rows: inputLines(text).map((value) => [value, value.replace(matcher, replacementText)])
    };
}
